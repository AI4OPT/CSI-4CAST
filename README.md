...
> - **General-purpose tuning framework included**: [`ahpt/`](ahpt/) is a standalone, reusable Optuna-based hyperparameter tuning framework designed for any deep learning project — it handles search-space definition, SLURM job orchestration, trial pruning, and dashboard monitoring out of the box. The CSI prediction integration in [`src/cp/tune/`](src/cp/tune/) demonstrates how to adapt it to a specific task. A sample Optuna study is provided in [`z_artifacts/outputs/ahpt/ablation/predictor/lstm_replace/`](z_artifacts/outputs/ahpt/ablation/predictor/lstm_replace/).
