...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes: int, ratio: int = 4):
        ...
        fc1 = nn.Conv2d(in_planes, in_planes // ratio, 1, bias=False)
        relu1 = nn.ReLU()
        fc2 = nn.Conv2d(in_planes // ratio, in_planes, 1, bias=False)
        sigmoid = nn.Sigmoid()
...
class ShuffleBlockCA(nn.Module):
    def __init__(
        ...
        # channel attention
        self.ca = ChannelAttention(in_planes=in_channels, ratio=ca_ratio)
...
class CSIEmbeddingShuffleNet(nn.Module):
    def __init__(
        ...
        self.res_ca_ratio = res_ca_ratio
        self.hist_len = hist_len
        self.dim_data = dim_data
        ...
        for _ in range(num_res_layers):
            list_RB_delay.append(ShuffleBlockCA(in_channels=res_dim, groups=res_groups, ca_ratio=res_ca_ratio))
            list_RB_freq.append(ShuffleBlockCA(in_channels=res_dim, groups=res_groups, ca_ratio=res_ca_ratio))
...
class Model(nn.Module):
    def __init__(
        ...
        self.embedding = CSIEmbeddingShuffleNet(
            ...
            res_ca_ratio=embedding_ca_ratio,
            hist_len=hist_len,
            dim_data=dim_data,
...
class MODEL_tdd_pl(BaseCSIModel):
