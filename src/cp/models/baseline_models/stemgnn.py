...
class StockBlockLayer(nn.Module):
    ...
    def spe_seq_cell(self, input):
        batch_size, _, _, node_cnt, time_step = input.size()
...
class STEMGNN_Predictor(nn.Module):
    def __init__(
        ...
        self.unit = embedded_dim
        self.stack_cnt = n_stacks
        self.alpha = leaky_rate
        self.time_step = window_size
        self.h = horizon
        ...
        self.fc = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Linear(int(self.time_step), self.horizon * 1),
        )
    ...
    def forward(self, x):
        mul_L, _ = self.latent_correlation_layer(x)
        ...
        # [batch_size, d_dim, pred_len] -> [batch_size, pred_len, d_dim]
...
if __name__ == "__main__":
    ...
    model = STEMGNN_Predictor(n_stacks=2, multi_layer=1)
    y = model(x)
    print(y.shape)
