...
from src.utils.vis_utils import order_models_for_scenario


def _format_delay_spread_ns(ds: float) -> str:
    """Format delay spread (seconds) as an integer ns label."""
    return f"{int(round(ds * 1e9))}ns"
...
def create_scenario_table_cm(
    ...
    table = table.reindex(index=order_models_for_scenario(table.index.tolist(), scenario_name))
...
Columns: A, C, D (Regular) | B, E (Generalization)"""
        ...
        all_ds_ns = [_format_delay_spread_ns(ds) for ds in LIST_DELAY_SPREAD_GEN]
        regular_ds_ns = [_format_delay_spread_ns(ds) for ds in LIST_DELAY_SPREAD]
        generalization_ds_ns = [
            _format_delay_spread_ns(ds) for ds in LIST_DELAY_SPREAD_GEN if ds not in LIST_DELAY_SPREAD
        ]

        filter_description = f"""Data: Filtered for delay spread analysis:
  - is_gen = True (generalization data only)
  - noise_type = 'vanilla'
  - delay_spread in {all_ds_ns} - analyzing these
  - min_speed in {LIST_MIN_SPEED_TEST} (regular)
  - channel_model in {LIST_CHANNEL_MODEL} (regular)
Columns: {', '.join(regular_ds_ns)} (Regular) | {', '.join(generalization_ds_ns)} (Generalization)"""
