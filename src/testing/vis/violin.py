...
def create_violin_plot(
    ...
    for _, row in scenario_combinations.iterrows():
        ...
        # Shared rank-axis anchors for alignment:
        # rank=1 should be near top; rank=max_rank near bottom.
        max_rank = int(violin_df["rank"].max()) if len(violin_df) > 0 else len(models_sorted)
        rank_axis_bottom = max_rank + 1
        rank_axis_top = 0.5
        ...
        ax1.set_ylim(rank_axis_bottom, rank_axis_top)
        ax1.set_yticks(range(1, max_rank + 1))

        # Configure secondary y-axis so:
        # 0% aligns with rank=max_rank and 100% aligns with rank=1.
        frac_rank_max = (max_rank - rank_axis_bottom) / (rank_axis_top - rank_axis_bottom)
        frac_rank_1 = (1 - rank_axis_bottom) / (rank_axis_top - rank_axis_bottom)
        frac_span = frac_rank_1 - frac_rank_max

        if frac_span == 0:
            ax2.set_ylim(0, 100)
        else:
            y2_span = 100 / frac_span
            y2_min = -frac_rank_max * y2_span
            y2_max = y2_min + y2_span
        ...
        # Remove all legends (Seaborn may auto-create one from hue)
