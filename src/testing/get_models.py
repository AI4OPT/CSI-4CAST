...
# Models that do not use Lightning checkpoints.
MODELS_NO_CHECKPOINT = {"NP", "AR", "PAD", "WIENER"}

# Models that require pre-estimated statistical parameter files.
MODELS_WITH_PARAM_FILE = {"AR", "WIENER"}
...
def get_ckpt_path(model_name: str, scenario: str) -> str | None:
    ...
    Returns:
        str | None: Path to checkpoint file, None for non-checkpoint baselines

    """
    if model_name in MODELS_NO_CHECKPOINT:
        return None
...
def _get_eval_model(model_name: str, device: torch.device, scenario: str, ckpt_path: str | None = None):
    ...
    """
    # Construct full model class name (e.g., "RNN_TDD", "NP_FDD", "AR_TDD", "WIENER_FDD")
    model_class_name = f"{model_name}_{scenario}"
    if not hasattr(PREDICTORS, model_class_name):
        raise ValueError(
            f"Model variant {model_class_name} is not registered in PREDICTORS. "
            "This usually means the model is not supported for this scenario."
        )
    model_class = getattr(PREDICTORS, model_class_name)

    if ckpt_path is not None:
        # Learning-based models (Lightning checkpoints)
        model = model_class.load_from_checkpoint(ckpt_path)
    else:
        # Baseline models
        param_path = get_param_path(model_name=model_name, scenario=scenario)
        if param_path is not None:
            param_file = Path(param_path)
            if not param_file.exists():
                raise FileNotFoundError(
                    f"Missing parameter file for {model_name}_{scenario}: {param_file}. "
                    "Run statistical parameter estimation first."
                )
            model = model_class(param_path=param_path)
        else:
            model = model_class()
...
def get_eval_model(model_name: str, device: torch.device, scenario: str):
    ...
    param_path = get_param_path(model_name=model_name, scenario=scenario)
    logger.info(
        f"Loading model {model_name} for scenario {scenario} | checkpoint: {ckpt_path} | params: {param_path}\n"
    )
