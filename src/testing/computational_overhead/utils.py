...
- Inference memory measurements (GPU peak, CPU RSS, model footprint)
- Optional energy measurements (GPU via NVML, CPU via RAPL)
...
def count_flops(model: torch.nn.Module | pl.LightningModule, input_tensor: torch.Tensor) -> tuple[int, str | None]:
    ...
    try:
        ...
        deepspeed_err = str(e)
        print(f"DeepSpeed FLOPs profiler failed: {deepspeed_err}")

    # Fallback: PyTorch's built-in FlopCounterMode (handles complex tensors).
    # FlopCounterMode hooks ATen ops during the actual forward pass, so it
    # works with complex inputs (unlike DeepSpeed which creates float32 inputs).
    # For complex tensors, FlopCounterMode counts complex MACs at the same
    # dimensionality as real MACs, but each complex MAC requires 4 real
    # multiplications + 2 real additions. We apply a 4x multiplier to make
    # the result comparable with real-valued FLOPs from DeepSpeed.
    try:
        ...
        if total > 0:
            if is_complex:
                total *= 4
                print(f"FlopCounterMode succeeded: {total} FLOPs (4x complex correction applied)")
            else:
                print(f"FlopCounterMode succeeded: {total} FLOPs")
            return int(total), None
        print("FlopCounterMode returned 0 FLOPs")
        return 0, None
    except Exception as e:
        print(f"FlopCounterMode also failed: {e}")
        return -1, f"DeepSpeed: {deepspeed_err} | FlopCounterMode: {e}"
...
def get_model_param_memory_bytes(model: torch.nn.Module, include_buffers: bool = True) -> dict[str, int]:
    """Return memory footprint of model parameters and buffers in bytes."""
    param_bytes = sum(p.numel() * p.element_size() for p in model.parameters())
    buffer_bytes = sum(b.numel() * b.element_size() for b in model.buffers()) if include_buffers else 0

    return {
        "param_bytes": int(param_bytes),
        "buffer_bytes": int(buffer_bytes),
        "param_plus_buffer_bytes": int(param_bytes + buffer_bytes),
    }


def get_model_device_memory_bytes(model: torch.nn.Module, include_buffers: bool = True) -> dict[str, int]:
    """Return model memory split by tensor device type in bytes."""
    cpu_bytes = 0
    cuda_bytes = 0
    other_bytes = 0
    ...
    for tensor in tensors:
        ...
        if tensor.device.type == "cpu":
            ...
            cuda_bytes += tensor_bytes
        else:
            other_bytes += tensor_bytes

    return {
        "cpu_bytes": int(cpu_bytes),
        "gpu_bytes": int(cuda_bytes),
        "other_bytes": int(other_bytes),
        "total_bytes": int(cpu_bytes + cuda_bytes + other_bytes),
    }


def measure_peak_gpu_memory_inference(
    model: torch.nn.Module,
    input_data: torch.Tensor,
    device: torch.device,
    *,
    ...
    """Measure peak CUDA memory usage during repeated inference (bytes)."""
    if device.type != "cuda":
        return {
    ...
    return {
        ...
        "peak_allocated_delta_bytes": int(max(0, peak_allocated - baseline_allocated)),
        "peak_reserved_delta_bytes": int(max(0, peak_reserved - baseline_reserved)),
    }


def measure_peak_cpu_rss_bytes(
    run_fn: Callable[[], None],
    *,
    warmup_iterations: int = 10,
    measure_iterations: int = 200,
) -> dict[str, int] | None:
    """Measure peak process RSS increase during repeated inference (bytes)."""
    ...
    return {
        "rss_baseline_bytes": int(baseline_rss),
        "rss_peak_bytes": int(peak_rss),
        "rss_peak_delta_bytes": int(peak_rss - baseline_rss),
    }


def estimate_gpu_energy_per_inference_nvml(
    model: torch.nn.Module,
    input_data: torch.Tensor,
    device: torch.device,
    *,
    warmup_iterations: int = 30,
    measure_iterations: int = 150,
    sample_interval_ms: int = 1,
    subtract_idle: bool = True,
    idle_seconds: float = 0.5,
) -> dict[str, float | str | bool]:
    """Estimate per-inference GPU energy using NVML power sampling."""
    default = {
        "available": False,
        "energy_per_inference_J": float("nan"),
    ...
    if device.type != "cuda":
        default["error"] = "cuda_device_not_used"
        return default

    def read_power_watts(handle) -> float:
    ...
    try:
        ...
        default["error"] = f"nvml_init_failed: {exc}"
        return default
    ...
    try:
        ...
        if subtract_idle:
            ...
            while (time.perf_counter() - start_time) < idle_seconds:
                idle_samples.append(read_power_watts(handle))
                time.sleep(interval_s)
            idle_power = sum(idle_samples) / max(len(idle_samples), 1)
        ...
        with torch.inference_mode():
            ...
            def sampler() -> None:
                samples.append((time.perf_counter(), read_power_watts(handle)))
        ...
        if len(samples) < 2:
            default["error"] = "insufficient_power_samples"
            return default

        energy_j = 0.0
        for (t0, p0), (t1, p1) in pairwise(samples):
            if subtract_idle:
                p0 = max(p0 - idle_power, 0.0)
                p1 = max(p1 - idle_power, 0.0)
        ...
        return {
            "available": True,
            "energy_per_inference_J": float(energy_j / float(measure_iterations)),
        ...
        default["error"] = f"nvml_sampling_failed: {exc}"
        return default
    finally:
        nvmlShutdown()


def _rapl_package_energy_paths() -> list[tuple[str, Path, Path]]:
    """Return available top-level RAPL package energy counter paths."""
    base_path = Path("/sys/class/powercap")
    if not base_path.exists():
        return []

    paths: list[tuple[str, Path, Path]] = []
    for pkg_dir in base_path.glob("*rapl:*"):
        parts = pkg_dir.name.split(":")
        if len(parts) != 2 or not parts[1].isdigit():
            continue

        energy_path = pkg_dir / "energy_uj"
        max_range_path = pkg_dir / "max_energy_range_uj"
        name_path = pkg_dir / "name"

        if energy_path.exists() and max_range_path.exists():
            name = name_path.read_text().strip() if name_path.exists() else pkg_dir.name
            paths.append((name, energy_path, max_range_path))

    return paths


def _read_int(path: Path) -> int:
    """Read and parse an integer value from a sysfs path."""
    return int(path.read_text().strip())


def measure_cpu_package_energy_per_inference_rapl(
    run_fn: Callable[[], None],
    *,
    warmup_iterations: int = 30,
    measure_iterations: int = 200,
) -> dict[str, float | str | bool] | None:
    """Measure per-inference CPU package energy using Linux RAPL counters."""
    packages = _rapl_package_energy_paths()
    if not packages:
        return None
    ...
    try:
        ...
        e0 = {name: _read_int(energy_path) for name, energy_path, _ in packages}
        ...
        e1 = {name: _read_int(energy_path) for name, energy_path, _ in packages}

        total_delta_uj = 0
        for name, _, max_range_path in packages:
            max_uj = _read_int(max_range_path)
            delta = e1[name] - e0[name]
            if delta < 0:
                delta += max_uj
            total_delta_uj += delta

        energy_j = total_delta_uj / 1e6
        duration_s = t1 - t0

        return {
            "available": True,
            "energy_per_inference_J": float(energy_j / float(measure_iterations)),
            "avg_power_W": float(energy_j / duration_s) if duration_s > 0 else float("nan"),
            "duration_s": float(duration_s),
            "energy_source": ",".join(name for name, _, _ in packages),
            "error": "",
        }
    except Exception as exc:
        return {
            ...
            "duration_s": float("nan"),
            "energy_source": "",
            "error": f"rapl_measurement_failed: {exc}",
        }
