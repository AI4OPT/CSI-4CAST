...
def log_table(logger: logging.Logger, df: pd.DataFrame, title: str = "") -> None:
    """Log a DataFrame as a formatted table, aligned correctly with the logger prefix.

    A leading newline pushes the table below the logger's location prefix so
    that the header row and data columns stay aligned.

    Args:
        logger (logging.Logger): Logger instance
        df (pd.DataFrame): Table to log
        title (str): Optional title printed above the table

    """
    header = f"  {title}" if title else ""
    logger.info("%s\n%s", header, df.to_string())
